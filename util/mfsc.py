...
class TIDIGIT_Converter():
    ...
    def convert_tidigit_own(self, PATH, item_name, sample_rate, timeframes, freq_bins):
        ...
        # taking subset of all samples
        samples_sub = samples[0:5]

        audios = [None] * len(samples_sub)
        sample_rates = [sample_rate] * len(samples_sub)
        for index, item in tqdm(enumerate(samples_sub)):
